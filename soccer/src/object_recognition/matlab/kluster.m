...
%        automatically; Unlike many other implementations, this one is
%        immune to problems caused by multimodal densities with widely
%        separated modes (see example). The estimation does not deteriorate
%        for multimodal densities, because we never assume a parametric
%        model for the data.
% INPUTS:
%     data    - a vector of data from which the density estimate is
%     constructed;
%          n  - the number of mesh points used in the uniform
%          discretization of the
%               interval [MIN, MAX]; n has to be a power of two; if n is
%               not a power of two, then n is rounded up to the next power
%               of two, i.e., n is set to n=2^ceil(log2(n)); the default
%               value of n is n=2^12;
%   MIN, MAX  - defines the interval [MIN,MAX] on which the density
...
%             - If no output is requested, then the code automatically
%             plots a graph of
...
% Re-order elements of each column according to equations (5.93) and (5.94)
% in Jain
out = zeros(nrows,1);
